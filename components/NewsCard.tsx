...
import { Clock, Share2, ExternalLink, StickyNote, X, Save, Bookmark, BookmarkCheck } from 'lucide-react';
...
const NewsCard = memo(({ news, isActive, onAutoFlip, isAutoFlipEnabled }: NewsCardProps) => {
    ...
    useEffect(() => {
        ...
        if (isActive) {
            fetch('/api/saved-news')
                .then(res => res.json())
                .then((data: any[]) => {
                    if (data.some(item => item.id === news.id)) {
                        setIsSaved(true);
                    }
                })
    ...
    const handleSaveVideo = async () => {
        ...
        try {
            ...
            console.error("Failed to save/unsave news", error);
        } finally {
            setIsSaving(false);
        }
    };
    ...
    return (
        <div className="relative w-full h-full flex items-center justify-center p-4 overflow-hidden z-10">
            ...
            <motion.div
                ...
                className="glass-premium relative z-10 w-full max-w-2xl rounded-3xl p-8 md:p-12 flex flex-col min-h-[600px] transition-all duration-500"
            >
                <div className="flex items-center justify-between mb-6">
                ...
                <div className="relative mb-8 group">
                    <div className="absolute -inset-1 bg-accent-500/20 blur-xl rounded-[2rem] opacity-0 group-hover:opacity-100 transition-all duration-700 pointer-events-none" />
                    <div className="w-full aspect-[16/9] rounded-2xl overflow-hidden shadow-2xl border border-white/5 relative z-10">
                        <img
                            src={news.imageUrl}
                            alt={news.title}
                ...
                <div className="flex items-center justify-between mt-auto pt-6 border-t border-white/5">
                    <div className="flex gap-4">
                        ...
                        <button
                            ...
                            className={`p-2.5 rounded-full border border-white/10 ${isSaved ? `text-${accentColor}-400 bg-${accentColor}-400/10 border-${accentColor}-400/30` : 'text-slate-300'} hover:text-${accentColor}-300 hover:border-${accentColor}-300/30 hover:bg-${accentColor}-400/10 transition-all duration-300 flex items-center gap-2 group`}
                        >
                            {isSaved ? <BookmarkCheck size={20} className="group-hover:scale-110 transition-transform" /> : <Bookmark size={20} className="group-hover:scale-110 transition-transform" />}
                        </button>
                    </div>
                    <button
                        onClick={handleShare}
                        className="p-2.5 text-slate-400 hover:text-white hover:bg-white/10 rounded-full transition-all duration-300 transform hover:rotate-12"
                        title="Share"
                    >
                        <Share2 size={24} />
                    </button>
